...
### Phase 5: 基于模板维度的需求分析（模板融合）

根据复杂度等级选择合适的分析框架，并基于模板提供的维度进行深度需求分析：
...
1. **Epic** (`templates/requirements/agile/Epic.md`)
   - 分析维度：战略举措、痛点、价值、目标用户、背景现状、MVP规划、成效指标、风险依赖
   - 从战略层面分析需求

2. **Feature** (`templates/requirements/agile/Feature.md`)
   - 分析维度：需求场景、用户痛点、需求洞察、特性设计、价值度量、依赖风险
   - 从特性层面分析需求

3. **Story** (`templates/requirements/agile/Story.md`)
   - 分析维度：**INVEST原则**（独立、可协商、有价值、可估算、规模小、可验证）
   - 从用户故事层面深入分析需求
   - 用户视角分析：作为 <用户角色>, 我想要 <完成 xxx 活动>, 以便于 <实现 xxx 价值>

4. **Task** (`templates/requirements/agile/Task.md`)
   - 分析维度：具体工作项、完成标准、依赖关系
   - 从任务层面分析实现细节
...
#### 步骤2：读取对应的模板文件，提取分析维度

**瀑布流模式（高复杂度 ≥12分）**：
```bash
# 使用 Read 工具读取以下模板文件，提取分析维度
...
**敏捷模式分析维度示例**：
```markdown
从 Story 模板提取的 INVEST 维度：
- Independent（独立）：检查用户故事是否独立
- Negotiable（可协商）：检查需求是否可讨论
- Valuable（有价值）：检查是否对用户有价值
- Estimable（可估算）：检查工作量是否可估算
- Small（规模小）：检查规模是否适中
- Testable（可验证）：检查是否可验证
```
...
**记住**: 清晰的需求是成功交付的一半。不要急于进入下一阶段，确保需求完整准确。模板提供分析维度和框架，帮助你进行全面深入的需求分析，而不是限制你的思维。
